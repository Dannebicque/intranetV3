...
//    }
    ...
    public function update(?string $keyEduSign, ?int $opt, string $date = ''): ?array
    {
        $response = ['success' => false, 'error' => '']; // Initialisation du tableau de réponse
...
//        $bilan = [];
        ...
        foreach ($diplomes as $diplome) {
            ...
            list($start, $end) = $this->calculateStartEndDates($opt, $date);
...
//        return $bilan ?? null;
    ...
    private function calculateStartEndDates(?int $opt, string $date): array
    {
        switch ($opt) {
            case 1:
                $start = Carbon::today();
    ...
    public function processEvent(Diplome $diplome): void
    {
        if (!($this->evenement->matiere === null || $this->evenement->matiere === "Inconnue" || $this->evenement->groupeObjet === null || $this->evenement->personnelObjet === null || $this->evenement->semestre === null)) {
            $course = (new IntranetEdtEduSignAdapter($this->evenement))->getCourse();
            $eduSignCourse = $this->apiCours->getCourseIdByApiId($this->evenement->id, $this->cleApi);
