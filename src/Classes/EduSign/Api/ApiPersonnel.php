...
class ApiPersonnel
{
    ...
    public function addEnseignant(EduSignEnseignant $enseignant, Personnel $personnel, Diplome $diplome, $cleApi): mixed
    {
    ...
    public function getEnseignantByEmail(?string $mailuniv, string $cleApi): mixed
    {
        $client = HttpClient::create();

        $response = $client->request('GET', 'https://ext.edusign.fr/v1/professor/by-email/' . $mailuniv, [
            'headers' => [
                'Content-Type' => 'application/json',
                'Authorization' => 'Bearer ' . $this->getCleApi->getCleApi($cleApi),
            ]
