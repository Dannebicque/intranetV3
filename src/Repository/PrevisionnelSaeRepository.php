...
class PrevisionnelSaeRepository extends PrevisionnelRepository
{
    ...
    public function findPrevisionnelEnseignantComplet(Personnel $personnel, $annee): array
    {
        return $this->createQueryBuilder('p')
            ->innerJoin(ApcSae::class, 'e', 'WITH', 'p.id_matiere = e.id')
            ->where('p.annee = :annee')
    ...
    public function findPrevisionnelEnseignantDepartementAnnee(
        ...
        return $this->createQueryBuilder('p')
            ...
            ->innerJoin(ApcSae::class, 'm', 'WITH', 'p.id_matiere = m.id')
            ->select('p.id as id_previsionnel, p.annee, p.referent, p.nbHCm, p.nbHTd, p.nbHTp, p.nbGrCm, p.nbGrTd, p.nbGrTp, m.id as id_sae, m.libelle, m.codeSae, pers.id as id_personnel, pers.nom, pers.prenom, pers.numeroHarpege, pers.mailUniv, pers.nbHeuresService, s.id as id_semestre, s.libelle as libelle_semestre, a.id as id_annee, a.libelle as libelle_annee, d.id as id_diplome, d.libelle as libelle_diplome')
