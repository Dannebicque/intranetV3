...
class Uploader
{
        ...
        if (is_array($field) && in_array($field, $_FILES)) {
            ...
            if (!is_array($this->field['name'])) {
                $this->field = array_merge($this->field, array(
                    "name"     => array($this->field['name']),
                    "tmp_name" => array($this->field['tmp_name']),
                    "type"     => array($this->field['type']),
                    "error"    => array($this->field['error']),
                    "size"     => array($this->field['size'])
                ));
            }
    ...
    private function validate($file = null)
    {
        ...
        if ($file == null) {
            ...
            if (!file_exists($options['uploadDir']) && !is_dir($options['uploadDir']) && mkdir($options['uploadDir'],
                    750, true)) {
            ...
            if (@$field['error'][$file['index']] > 0 && array_key_exists($field['error'][$file['index']],
                    $this->error_messages)) {
        ...
        if ($validate) {
            ...
            for ($i = 0; $i < count($field['name']); $i++) {
                ...
                $metas['name'] = $this->generateFileName($this->options['title'], array(
                    'name'      => substr($metas['old_name'], 0,
                        (!empty($metas['extension']) ? -(strlen($metas['extension']) + 1) : strlen($metas['old_name']))),
                    'size'      => $metas['size'],
                ...
                if ($isAddMoreMode) {
                    ...
                    $matches = preg_grep('/^' . (count($addMoreMatches[$field['name'][$i]]) - 1) . '\:\/\/' . $field['name'][$i] . '/i',
                        $removed_files);
                    if (count($matches) == 1) {
                ...
                if (!$is_file_removed && $this->validate(array_merge($metas,
                        array('index' => $i, 'tmp' => $tmp_name))) && $this->uploadFile($tmp_name, $metas['file'])) {
    ...
    private function removeFiles()
    {
        ...
        if ($this->options['removeFiles'] !== false) {
            foreach ($_POST as $key => $value) {
                preg_match((is_string($this->options['removeFiles']) ? $this->options['removeFiles'] : '/jfiler-items-exclude-' . $this->field['Field_Name'] . '-(\d+)/'),
                    $key, $matches);
    ...
    private function generateFileName($conf, $file, $skip_replace_check = false)
    {
        ...
        $random_string = substr(str_shuffle("_0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"), 0,
            $length ? $length : 10);
    ...
    private function _onUpload()
    {
        ...
        return $this->options['onUpload'] && function_exists($this->options['onUpload']) ? $this->options['onUpload'](@$arguments[0],
            @$arguments[1]) : null;
    }
